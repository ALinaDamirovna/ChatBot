...
bot = telebot.TeleBot('5397416814:AAHSyWYIEV7laoift6L9IgdRmF5xdNuO-C4')  # here you need your personal token
...
def welcome(message):
    ...
    try:
        bot.send_message(message.chat.id,
                         "Добро пожаловать, {0.first_name}!\nЯ - <b>{1.first_name}</b>, бот созданный для вывода информации о расписании факультета ИВМиИТ.".format(
...
def help(message):
    logger.info(
        "{0}|{1}|{2}: {3}".format(message.chat.username, message.chat.first_name, message.chat.last_name, message.text))
    header = "Я - <b>{0.first_name}</b>, бот созданный для вывода информации о расписании факультета ИВМиИТ.".format(
        bot.get_me())
    contact = "Если появились вопросы, встретили баг или неточность, писать на почту adgaripova@stud.kpfu.ru";
